...
SCRIPT = REPO_ROOT / "tests" / "device" / "rfhook_goal05_stalker_writer.js"
    ...
    def wait_for(self, pattern, timeout=60):
        ...
        while time.monotonic() < deadline:
            ...
            try:
                ...
                continue
            if "[JS error]" in line or "[stalker] transform callbacks did not stop" in line:
...
def main():
    parser = argparse.ArgumentParser(description="Run Goal 05 Stalker ARM64 writer device regression")
    ...
    with tempfile.TemporaryDirectory(prefix="rf-goal05-") as temporary:
        temporary = Path(temporary)
        fixture = temporary / "librf_goal05_stalker_writer.so"
        host = temporary / "rf_g05_host"
        run([
            str(clang), "-shared", "-fPIC", "-O2", "-g", "-Wl,--build-id=none",
            "-Wl,-soname,librf_goal05_stalker_writer.so",
            str(FIXTURES / "goal05_stalker_writer.c"), "-o", str(fixture),
        ])
        run([
            str(clang), "-fPIE", "-pie", "-O2", "-g", "-Wl,--build-id=none",
            str(FIXTURES / "goal05_host.c"), "-ldl", "-o", str(host),
        ])

        device_rustfrida = f"{DEVICE_ROOT}/rustfrida-goal05"
    ...
    root_shell("kill $(pidof rf_g05_host) 2>/dev/null || true")
    tombstones_before = root_shell(
        "for f in /data/tombstones/*; do [ -f \"$f\" ] && stat -c '%n:%Y:%s' \"$f\"; done; true",
        capture=True,
    ).stdout.splitlines()
    start = root_shell(
        f"sh -c 'nohup {DEVICE_ROOT}/rf_g05_host >{DEVICE_ROOT}/rf_g05_host.log 2>&1 & echo $!'",
        capture=True,
    )
    ...
    if failure is not None:
        raise failure
    print(f"[goal05-runner][PASS] target pid {host_pid} survived reload; Stalker writer state retired safely")
