...
SCRIPT = REPO_ROOT / "tests" / "device" / "rfhook_goal02_diagnostics.js"
...
class Session:
    ...
    def wait_for(self, pattern, timeout=45):
...
def main():
    parser = argparse.ArgumentParser(description="Run Goal 02 diagnostics device regression")
    ...
    with tempfile.TemporaryDirectory(prefix="rf-goal02-") as temporary:
        temporary = Path(temporary)
        target_so = temporary / "librf_goal01_unload.so"
        control_so = temporary / "librf_goal01_control.so"
        host = temporary / "rf_g02_host"
        run([
            str(clang), "-shared", "-fPIC", "-O2", "-g", "-Wl,--build-id=none",
            "-Wl,-soname,librf_goal01_unload.so",
            str(FIXTURES / "module_unload_target.c"), "-o", str(target_so),
        ])
        run([
            str(clang), "-shared", "-fPIC", "-O2", "-g", "-fno-omit-frame-pointer",
            "-fno-optimize-sibling-calls", "-Wl,--build-id=none",
            "-Wl,-soname,librf_goal01_control.so",
            str(FIXTURES / "module_unload_control.c"), "-ldl", "-o", str(control_so),
        ])
        run([
            str(clang), "-fPIE", "-pie", "-O2", "-g", "-Wl,--build-id=none",
            str(FIXTURES / "module_unload_host.c"), "-ldl", "-o", str(host),
        ])

        device_rustfrida = f"{DEVICE_ROOT}/rustfrida-goal02"
        device_script = f"{DEVICE_ROOT}/{SCRIPT.name}"
        for local, remote in (
            (target_so, f"{DEVICE_ROOT}/{target_so.name}"),
            (control_so, f"{DEVICE_ROOT}/{control_so.name}"),
    ...
    root_shell("kill $(pidof rf_g02_host) 2>/dev/null || true")
    tombstones_before = root_shell(
        "for f in /data/tombstones/*; do [ -f \"$f\" ] && stat -c '%n:%Y:%s' \"$f\"; done; true",
        capture=True,
    ).stdout.splitlines()
    start = root_shell(
        f"sh -c 'nohup {DEVICE_ROOT}/rf_g02_host >{DEVICE_ROOT}/rf_g02_host.log 2>&1 & echo $!'",
        capture=True,
    )
    ...
    try:
        ...
        for round_number in (1, 2):
            session.wait_for(r"\[goal02\]\[READY\] diagnostics verified")
            if round_number == 1:
                session.send("%reload")
        session.send("exit")
        ...
        for marker in ("Fatal signal", "SIGSEGV", "SIGABRT", "cleanup timeout"):
            if marker in output:
    ...
    if failure is not None:
        raise failure
    print(f"[goal02-runner][PASS] target pid {host_pid} survived reload; no new tombstone")
