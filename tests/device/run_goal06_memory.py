...
SCRIPT = REPO_ROOT / "tests" / "device" / "rfhook_goal06_memory.js"
    ...
    def wait_for(self, pattern, timeout=60):
        ...
        while time.monotonic() < deadline:
            ...
            try:
                ...
                continue
            if "[JS error]" in line or "memory scan callbacks did not stop" in line:
...
def main():
    parser = argparse.ArgumentParser(description="Run Goal 06 Memory advanced API device regression")
    ...
    with tempfile.TemporaryDirectory(prefix="rf-goal06-") as temporary:
        temporary = Path(temporary)
        fixture = temporary / "librf_goal06_memory.so"
        host = temporary / "rf_g06_host"
        run([
            str(clang), "-shared", "-fPIC", "-O2", "-g", "-Wl,--build-id=none",
            "-Wl,-soname,librf_goal06_memory.so",
            str(FIXTURES / "goal06_memory.c"), "-pthread", "-o", str(fixture),
        ])
        run([
            str(clang), "-fPIE", "-pie", "-O2", "-g", "-Wl,--build-id=none",
            str(FIXTURES / "goal06_host.c"), "-ldl", "-o", str(host),
        ])

        device_rustfrida = f"{DEVICE_ROOT}/rustfrida-goal06"
    ...
    root_shell("kill $(pidof rf_g06_host) 2>/dev/null || true")
    tombstones_before = root_shell(
        "for f in /data/tombstones/*; do [ -f \"$f\" ] && stat -c '%n:%Y:%s' \"$f\"; done; true",
        capture=True,
    ).stdout.splitlines()
    start = root_shell(
        f"sh -c 'nohup {DEVICE_ROOT}/rf_g06_host >{DEVICE_ROOT}/rf_g06_host.log 2>&1 & echo $!'",
        capture=True,
    )
    ...
    try:
        ...
        for round_number in (1, 2):
            session.wait_for(r"\[goal06\]\[READY\] Memory advanced APIs verified")
            if round_number == 1:
                session.send("%reload")
        session.send("exit")
        if session.finish() != 0:
        ...
        if output.count("[goal06][READY]") != 2:
            raise RuntimeError("Goal 06 did not complete exactly once per runtime")
        if "MemoryAccessMonitor onAccess error" in output:
            raise RuntimeError("a monitor callback raised after teardown")
    ...
    if failure is not None:
        raise failure
    print(f"[goal06-runner][PASS] target pid {host_pid} survived reload; memory callbacks retired safely")
