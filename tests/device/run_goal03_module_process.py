...
SCRIPT = REPO_ROOT / "tests" / "device" / "rfhook_goal03_module_process.js"
...
def main():
    parser = argparse.ArgumentParser(description="Run Goal 03 Module/Process device regression")
    ...
    with tempfile.TemporaryDirectory(prefix="rf-goal03-") as temporary:
        temporary = Path(temporary)
        control = temporary / "librf_goal03_control.so"
        module = temporary / "librf_goal03_module.so"
        memfd = temporary / "librf_goal03_memfd.so"
        same_a = temporary / "same-a" / "librf_goal03_same.so"
        same_b = temporary / "same-b" / "librf_goal03_same.so"
        host = temporary / "rf_g03_host"
        same_a.parent.mkdir()
        same_b.parent.mkdir()

        common_so = [str(clang), "-shared", "-fPIC", "-O2", "-g", "-Wl,--build-id=none"]
        run([*common_so, "-Wl,-soname,librf_goal03_control.so", str(FIXTURES / "goal03_control.c"), "-ldl", "-o", str(control)])
        run([*common_so, "-Wl,-soname,librf_goal03_module.so", str(FIXTURES / "goal03_module.c"), "-o", str(module)])
        run([*common_so, "-Wl,-soname,librf_goal03_memfd.so", str(FIXTURES / "goal03_module.c"), "-DRF_GOAL03_VALUE=3303", "-o", str(memfd)])
        run([*common_so, "-Wl,-soname,librf_goal03_same.so", str(FIXTURES / "goal03_module.c"), "-DRF_GOAL03_VALUE=3101", "-o", str(same_a)])
        run([*common_so, "-Wl,-soname,librf_goal03_same.so", str(FIXTURES / "goal03_module.c"), "-DRF_GOAL03_VALUE=3102", "-o", str(same_b)])
        run([str(clang), "-fPIE", "-pie", "-O2", "-g", "-Wl,--build-id=none", str(FIXTURES / "goal03_host.c"), "-ldl", "-o", str(host)])
        ...
        for local, remote in (
            ...
            (memfd, f"{DEVICE_ROOT}/{memfd.name}"),
            (same_a, f"{DEVICE_ROOT}/rf-goal03-a/{same_a.name}"),
            (same_b, f"{DEVICE_ROOT}/rf-goal03-b/{same_b.name}"),
        ...
        root_shell(
            ...
            f"chmod 644 {DEVICE_ROOT}/librf_goal03_*.so {DEVICE_ROOT}/rf-goal03-a/*.so "
            f"{DEVICE_ROOT}/rf-goal03-b/*.so {device_script}"
        )

    root_shell("kill $(pidof rf_g03_host) 2>/dev/null || true")
    tombstones_before = root_shell(
        "for f in /data/tombstones/*; do [ -f \"$f\" ] && stat -c '%n:%Y:%s' \"$f\"; done; true",
        capture=True,
    ).stdout.splitlines()
    start = root_shell(
        f"sh -c 'nohup {DEVICE_ROOT}/rf_g03_host >{DEVICE_ROOT}/rf_g03_host.log 2>&1 & echo $!'",
        capture=True,
    )
    ...
    try:
        ...
        for round_number in (1, 2):
            session.wait_for(r"\[goal03\]\[READY\] module/process verified")
            if round_number == 1:
                session.send("%reload")
        session.send("exit")
        if session.finish(timeout=45) != 0:
    ...
    if failure is not None:
        raise failure
    print(f"[goal03-runner][PASS] target pid {host_pid} survived reload; no old callbacks or new tombstone")
